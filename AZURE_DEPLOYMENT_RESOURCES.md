...
**Status:** 🔴 CRITICAL - Deployment Workflow Misalignment Identified
...
### ⚠️ Deprecated/Uncertain Resources

| Resource Type | Resource Name | Status | Issue |
|---------------|---------------|--------|-------|
| **Function App** | `greenchainz-scraper` | ❓ Uncertain | Referenced in workflow but may not exist or be deprecated |
...
- **Status:** ✅ **CORRECT - Aligns with Container App architecture**

#### 2. **`greenchainz-container-AutoDeployTrigger-101801cc-7e0d-44eb-8afa-a3baea126d3d.yml`**
...
- **Status:** ✅ **CORRECT - Container App deployment**

#### 3. **`greenchainz-container-AutoDeployTrigger-118c312d-46c0-4285-af09-a03d38f9d660.yml`**
- **Purpose:** Alternative auto-deployment for backend Container App
- **Trigger:** Push to `main` branch or manual dispatch
- **Authentication:** OIDC with secrets (same as above)
- **Deployment Target:** `greenchainz-container` only
- **Resource Group:** `rg-greenchainz-prod-container` (different from above)
- **Action:** `azure/container-apps-deploy-action@v2`
- **Status:** ✅ **CORRECT - Container App deployment**
- **Note:** Duplicate workflow with different resource group - consider consolidating

---

### 🔴 **CRITICAL ISSUE: `main_greenchainz-scraper.yml`**

#### Problem Analysis

**Workflow Configuration:**
- **Deployment Action:** `Azure/functions-action@v1` ❌ (Function App action)
- **Target Resource:** `greenchainz-scraper` (Function App)
...
- **Node.js Version:** 20.x ✅ (Correct - matches requirements)
- **Build Process:** Builds entire repository root as Function App package ❌

**Issues Identified:**

1. ❌ **Wrong Azure Service Type:** Uses Function App deployment action, but architecture uses Container Apps
2. ❌ **Resource Uncertainty:** `greenchainz-scraper` Function App may not exist or is deprecated
3. ❌ **Secret Naming Mismatch:** Uses `AZUREAPPSERVICE_*` secrets (Function App pattern) vs `GREENCHAINZCONTAINER_*` (Container App pattern)
4. ❌ **Build Context Issue:** Builds entire repository as Function App, but no Azure Functions code structure exists in root
5. ❌ **No Function App Code:** Repository has no `host.json`, `local.settings.json`, or Azure Functions code in root
6. ⚠️ **Historical Context:** Documentation references `backend/functions/` directory that doesn't exist
...
### Priority 1: Deprecate or Remove Incorrect Workflow

**Option A: Remove Workflow (Recommended)**
```bash
# If no Function App is needed or exists
rm .github/workflows/main_greenchainz-scraper.yml
```

**Option B: Disable Workflow**
```yaml
# Rename to main_greenchainz-scraper.yml.disabled
# Or add to workflow:
on:
...
**Option C: Convert to Container App Deployment (If Scraper Needed)**
- Create new Container App resource: `greenchainz-scraper`
- Update workflow to use `azure/container-apps-deploy-action@v2`
- Create Dockerfile for scraper functionality
- Update secrets to match Container App pattern

### Priority 2: Consolidate Duplicate AutoDeploy Workflows

**Current Situation:**
- Two nearly identical workflows for `greenchainz-container`
- Different resource groups: `rg-greenchainz` vs `rg-greenchainz-prod-container`

**Recommendation:**
1. Determine which resource group is production
2. Keep one AutoDeploy workflow, delete the other
3. Rely primarily on `deploy-azure-cd.yml` for production

### Priority 3: Update Documentation

**Files to Update:**
- ✅ This file (`AZURE_DEPLOYMENT_RESOURCES.md`) - Created
- ⚠️ `.github/copilot-instructions.md` - Remove Function App references
- ⚠️ `docs/archive/SCRAPER_DEPLOYMENT_STATUS.md` - Mark as outdated/archived

---

## 📋 Deployment Workflow Decision Matrix

### When to Use Each Workflow

| Workflow | Use When | Don't Use When |
|----------|----------|----------------|
| `deploy-azure-cd.yml` | **Production deployments** to both frontend and backend | Testing individual services |
| `AutoDeployTrigger-*.yml` | Auto-deployments (Azure-managed) | Manual control needed |
| `main_greenchainz-scraper.yml` | ❌ **NEVER** (Deprecated) | ❌ Always - needs removal/fix |
...
❌ These secrets are likely unused and should be audited:
- `AZUREAPPSERVICE_CLIENTID_1FFCC2E636CB46C2AE2804DCC984C403`
- `AZUREAPPSERVICE_TENANTID_FFA2562F65A344B3B8ABCE2B0E8E2D36`
- `AZUREAPPSERVICE_SUBSCRIPTIONID_AB79C7B6EBF84254ADD1EC34639A4E92`
...
|----------|--------------|------------|-------|
| `deploy-azure-cd.yml` | Inherits from Dockerfile | ✅ 20.x | Uses Dockerfile node:20-alpine |
| `AutoDeployTrigger-*.yml` | Inherits from Dockerfile | ✅ 20.x | Uses Dockerfile node:20-alpine |
| `main_greenchainz-scraper.yml` | 20.x (explicit) | ✅ 20.x | **But workflow is deprecated** |
...
**Result:** ✅ All configurations align with Azure SDK requirements (Node.js >=20.0.0)
...
### Current State: ✅ Container Apps, ❌ Function Apps

**Production Architecture:**
- ✅ Frontend: `greenchainz-frontend` (Container App)
- ✅ Backend: `greenchainz-container` (Container App)
- ❌ Scraper: Not implemented (no Function App)

**Workflow Status:**
- ✅ `deploy-azure-cd.yml` - **Active, Correct**
- ⚠️ `AutoDeployTrigger-*.yml` - **Active, Duplicate (needs consolidation)**
- ❌ `main_greenchainz-scraper.yml` - **Incorrect, Needs Removal**

**Node.js Compliance:**
- ✅ All configurations use Node.js 20.x
- ✅ Meets Azure SDK requirements (>=20.0.0)
...
**Status:** 🔴 **ACTION REQUIRED** - Incorrect workflow must be fixed  
**Priority:** 🔥 **HIGH** - Deployment clarity is critical for production stability  
