// frontend/src/components/AutodeskViewer.tsx
...
const TOKEN_ENDPOINT = import.meta.env.VITE_APS_TOKEN_ENDPOINT || '/api/aps/token';
...
function GhostMode(): JSX.Element {
  return (
    <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden flex flex-col items-center justify-center">
      ...
      <p className="text-gray-500 text-lg font-medium">No 3D Model Available</p>
      <p className="text-gray-400 text-sm mt-2">Contact supplier for 3D specifications</p>
...
function ErrorState({ message }: { message: string }): JSX.Element {
  return (
    <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden flex flex-col items-center justify-center p-6">
      <div className="text-red-500 mb-4">
        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
        </svg>
      </div>
      <p className="text-gray-900 font-medium mb-2">Unable to Load 3D Model</p>
  ...
  useEffect(() => {
    ...
    const initViewer = async (): Promise<void> => {
      try {
        ...
        // Fetch token from API endpoint (configurable via VITE_APS_TOKEN_ENDPOINT)
