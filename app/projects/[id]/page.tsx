...
export default function ProjectDetailPage() {
  ...
  const [newMaterial, setNewMaterial] = useState({ name: "", quantity: "", unit: "", category: "" });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-start mb-8 border-b border-white/10 pb-8">
          <div>
            ...
            <p className="mt-4 text-gray-300 max-w-2xl">{project.description}</p>
          </div>
          ...
          {addingMaterial && (
            <div className="bg-white/5 border border-white/10 p-6 rounded-xl mb-6">
              <form onSubmit={handleAddMaterial} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <button type="submit" className="bg-teal-600 hover:bg-teal-500 text-white px-4 py-2 rounded h-[42px]">
                  Add
                </button>
              </form>
            </div>
                ...
                <tbody className="divide-y divide-white/10">
                  {materials.map((mat) => (
                    <tr key={mat.id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-6 py-4 text-gray-400">{mat.category || "-"}</td>
                      <td className="px-6 py-4">{mat.quantity} {mat.unit}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase
                          ${mat.status === 'planned' ? 'bg-gray-700 text-gray-300' : ''}
                          ${mat.status === 'rfq_sent' ? 'bg-blue-500/20 text-blue-400' : ''}
                        `}>
                          {mat.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        {mat.status === 'planned' && (
                          <Link
                            href={`/architect/rfq/new?project_id=${project.id}&material_id=${mat.id}&name=${encodeURIComponent(mat.name)}&qty=${mat.quantity}&unit=${mat.unit}&cat=${mat.category}`}
        ...
        <div>
          ...
          {rfqs.length === 0 ? (
             <div className="text-gray-500 italic">No RFQs sent for this project yet.</div>
          ) : (
            <div className="grid gap-4">
              {rfqs.map((rfq) => (
                ...
                >
                  <div>
                    <h3 className="font-semibold text-lg">{rfq.material_specs?.material_type || "Material"} Request</h3>
                    <div className="text-sm text-gray-400">
                      Sent: {formatShortDate(rfq.created_at)} • {rfq.quotes?.[0]?.count || 0} Quotes Received
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase
                        ${rfq.status === 'open' || rfq.status === 'pending' ? 'bg-green-500/20 text-green-400' : ''}
                        ${rfq.status === 'closed' ? 'bg-gray-700 text-gray-300' : ''}
                      `}>
                        {rfq.status}
                    </span>
