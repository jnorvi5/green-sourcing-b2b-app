    ...
    <div className="max-w-4xl mx-auto p-8 mb-20">
      <header className="flex justify-between items-center mb-10 pb-6 border-b">
        <div>
          ...
          <p className="text-gray-500">RFQ ID: {rfqId}</p>
        </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <p className="font-semibold">
                  {rfq.materials?.join(", ") || rfq.message}
                </p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase mb-1">
              ...
              <div>
                ...
                <p className="font-semibold">{rfq.project_location}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase mb-1">
                  Timeline
          ...
          <div className="p-6 border border-gray-200 rounded-2xl space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Crystal Designs Ltd.</p>
              </div>
            </div>
...
import type { RfqWithResponse, RfqResponse } from '@/types/rfq';
import { formatMaterialType, formatShortDate, getDeadlineUrgency, getDeadlineUrgencyColor, getDeadlineUrgencyIcon } from '@/lib/utils/formatters';
          ...
          <div className="flex-shrink-0">
             {rfq.rfq_response ? (
                <div className="px-6 py-4 rounded-xl bg-blue-500/10 border border-blue-500/30 text-center">
                  ...
                  <p className="text-2xl font-bold text-white">${rfq.rfq_response.quote_amount.toLocaleString()}</p>
                  <p className="text-xs text-gray-400 mt-1">on {formatShortDate(rfq.rfq_response.responded_at)}</p>
                </div>
             ) : (
               <button
            ...
            <section className="bg-white/5 rounded-xl border border-white/10 p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="bg-black/20 p-4 rounded-lg">
                  ...
                  <p className="text-lg font-medium">{formatMaterialType(rfq.material_specs.material_type)}</p>
                </div>
                <div className="bg-black/20 p-4 rounded-lg">
                  <p className="text-gray-500 text-sm mb-1">Quantity Needed</p>
                  <p className="text-lg font-medium">{rfq.material_specs.quantity.toLocaleString()} <span className="text-gray-400 text-base">{rfq.material_specs.unit}</span></p>
                </div>
              </div>
              ...
              {Object.keys(rfq.material_specs).length > 2 && (
                 <div className="mt-6">
                   <h3 className="text-sm font-medium text-gray-400 mb-3">Additional Specifications</h3>
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     {Object.entries(rfq.material_specs)
                       .filter(([key]) => !['quantity', 'unit', 'material_type'].includes(key))
                       .map(([key, value]) => (
            ...
            <section className="bg-white/5 rounded-xl border border-white/10 p-6">
              ...
              {rfq.message && (
                <div className="bg-black/20 p-4 rounded-lg mb-6">
                  <p className="text-gray-400 italic">"{rfq.message}"</p>
                </div>
              )}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                  ...
                  <span className="text-white font-medium">{rfq.budget_range || 'Not specified'}</span>
                </div>
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                  <span className="text-gray-400">Delivery Deadline</span>
                  <span className="text-white font-medium">{formatShortDate(rfq.delivery_deadline)}</span>
                </div>
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                   ...
                   <div className="flex gap-2">
                     {rfq.required_certifications.length > 0 ? (
                       rfq.required_certifications.map(c => (
                         <span key={c} className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-xs rounded-full border border-emerald-500/20">{c}</span>
                       ))
                     ) : (
               ...
               <div className="space-y-4">
                 <div>
                   ...
                   <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium border ${
                      rfq.rfq_response ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                      rfq.status === 'closed' ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' :
                 ...
                 <div>
                   ...
                   <div className="w-full bg-gray-700 rounded-full h-2.5 mb-1">
                     <div className="bg-teal-500 h-2.5 rounded-full" style={{ width: `${rfq.match_score || 0}%` }}></div>
                   </div>
                   <p className="text-right text-xs text-teal-400 font-bold">{rfq.match_score || 0}% Match</p>
                 </div>
                 ...
                 <div>
                    <p className="text-gray-400 text-sm">
                       Need help? <a href="#" className="text-teal-400 hover:underline">Contact Support</a>
                    </p>
                 </div>
               </div>
            </div>
