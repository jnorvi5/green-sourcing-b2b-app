        ...
        switch (event.type) {
            case 'payment_intent.succeeded': {
                ...
                if (paymentIntent.metadata?.type === 'rfq_deposit') {
                    const result = await stripeService.handlePaymentSuccess(paymentIntent);
                    if (!result.success) {
                        console.error(`[Stripe Webhook] Failed to handle payment success: ${result.error}`);
            ...
            case 'charge.dispute.created': {
                ...
                console.warn(`[Stripe Webhook] ⚠️ DISPUTE CREATED: ${dispute.id} for charge ${dispute.charge}`);
