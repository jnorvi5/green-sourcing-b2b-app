    ...
    try {
        ...
        if (tier !== 'admin') {
            ...
            if (quota.callsused >= callLimit) {
                monitoring.trackEvent('AIGateway_QuotaExceeded', { userId: String(userId), tier, type: 'calls' });
                return {
                    allowed: false,
                    remaining: 0,
            ...
            if (tokensUsed > 0 && (quota.tokensused + tokensUsed) > tokenLimit) {
                monitoring.trackEvent('AIGateway_QuotaExceeded', { userId: String(userId), tier, type: 'tokens' });
                return {
                    allowed: false,
                    remaining: 0,
                    resetAt: quota.periodendsat,
        ...
        const remaining = tier === 'admin' ? -1 : (limits.callsPerMonth - quota.callsused - 1);
        ...
        monitoring.trackException(error, { context: 'checkAndUpdateQuota', userId: String(userId) });
...
async function getEntitlements(userId) {
    ...
    return {
        ...
        quota: {
            ...
            remaining: tier === 'admin' ? -1 : Math.max(0, limits.callsPerMonth - (quota?.callsused || 0))
        }
    };
}
