...
export function SupplierDashboard() {
  ...
  async function loadDashboard(isRefresh = false) {
    try {
      ...
      if (supplierError) {
        // If no supplier profile exists, redirect to onboarding or show error
        if (supplierError.code === "PGRST116") { // JSON object requested, multiple (or no) rows returned
           // No profile found
           setError("No supplier profile found. Please complete onboarding.");
           // Potentially redirect to onboarding here
        } else {
           setError(
            "Failed to load supplier profile. Please ensure your account is set up correctly."
          );
        }
        logError("Error loading supplier:", supplierError);
        setLoading(false);
        setRefreshing(false);
        return;
      }
      ...
      const { data: productsData, error: productsError } = await supabase
        .from("products")
        .select(
          "id, supplier_id, name, description, material_type, application, certifications, sustainability_data, specs, images, epd_url, verified"
        )
      ...
      // Load incoming RFQs
      // Using supplier_id to find RFQs sent to this supplier
      const { data: rfqsData, error: rfqsError } = await supabase
        ...
        .select(
          ...
          created_at,
          buyer_id,
          users!rfqs_buyer_id_fkey(full_name, company_name)
        `
        )
        .eq("supplier_id", supplierData.id)
        .eq("status", "pending")
      ...
      // (Though 'pending' status on RFQ usually implies it hasn't been responded to,
      // but supplier might have responded and status not updated yet or logic differs)
      ...
      const transformedRfqs: IncomingRfq[] = unquotedRfqs.map(
        (rfq: Record<string, unknown>) => {
          ...
          const projectDetails = rfq["project_details"] as any;
          const materialType = projectDetails?.material_type || "N/A";
          ...
          return {
            ...
            match_score: 100, // Direct match via supplier_id implies 100% intentional match
      ...
      const transformedQuotes: SupplierQuote[] = (quotesData || []).map(
        (quote: Record<string, unknown>) => {
          ...
          return {
            ...
            status: (quote["status"] as string).toLowerCase() as "submitted" | "accepted" | "rejected",
            responded_at: quote["created_at"] as string, // Using created_at as responded_at
            rfq: {
              project_name:
      ...
      const acceptedQuotes = transformedQuotes.filter(
        (q) => q.status === "accepted"
      ).length;
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            {/* Scraped EPD data */}
            {profile?.scraped_data && profile.scraped_data.length > 0 && (
              <div className="mb-8">
                ...
                <div className="grid gap-4">
                  {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                  {profile.scraped_data.map((data: any) => (
                    <div
                      key={data.id}
              ...
              {incomingRfqs.length === 0 ? (
                <Card className="text-center py-12">
                  <CardContent>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Check back soon or improve your profile.
                    </p>
                  </CardContent>
                </Card>
              ) : (
              ...
              <Card className="hover:bg-muted/50 transition-colors cursor-pointer mb-4">
                <CardContent className="p-4 flex items-center gap-4">
                  <div className="w-12 h-12 rounded-lg bg-teal-500/10 flex items-center justify-center">
                    <FaLeaf className="w-6 h-6 text-teal-600" />
                  </div>
                  <div>
              ...
              <Card className="hover:bg-muted/50 transition-colors cursor-pointer mb-4">
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Add sustainability credentials
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-yellow-50 dark:bg-yellow-950/20 border-yellow-200 dark:border-yellow-900/50">
                <CardContent className="p-4">
                  ...
                  <p className="text-xs text-yellow-700 dark:text-yellow-600">
                    {stats.profileCompleteness}% complete. A complete profile
                    gets 3x more RFQ matches!
                  </p>
                </CardContent>
              </Card>
            )}
