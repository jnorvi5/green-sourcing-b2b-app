...
const nextConfig = {
  ...
  // 🟢 CRITICAL FIX: Webpack config for Edge Runtime crash
  webpack: (config, { isServer, nextRuntime }) => {
    // 1. Disable splitChunks for Edge Runtime to prevent double-initialization
    if (nextRuntime === 'edge') {
      config.optimization = {
        ...config.optimization,
        splitChunks: false,
    ...
    // 3. Suppress source map warnings
